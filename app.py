...
with st.sidebar.form("inputs"):
    ...
    port_cost = st.number_input("Cost per Port (₹)", min_value=0.0, value=100.0, step=1.0, format="%.2f")
    cable_len = st.number_input("Cable Length per Connection (m)", min_value=0.0, value=10.0, step=0.5, format="%.2f")
    cable_cost = st.number_input("Cost per Unit Cable (₹/m)", min_value=0.0, value=50.0, step=1.0, format="%.2f")
    st.sidebar.markdown("---")
    st.sidebar.markdown("### Images (optional)")
    st.sidebar.write("If no images are present in the app folder, upload them here.")
    uploaded_computer = st.file_uploader("computer.png (node icon)", type=["png","jpg","jpeg"], key="comp")
    uploaded_prof = st.file_uploader("professor.png (prof photo)", type=["png","jpg","jpeg"], key="prof")
    uploaded_abijith = st.file_uploader("abijith.jpeg (dev photo)", type=["png","jpg","jpeg"], key="ab1")
    uploaded_dharmayu = st.file_uploader("dharmyu.jpeg (dev photo)", type=["png","jpg","jpeg"], key="ab2")
    generate = st.form_submit_button("Generate Topology")
    
# -------------------
# Static assets fallback
# -------------------
def load_image_from_upload_or_disk(uploaded_file, disk_path):
...
# look for images in local folder
computer_img = load_image_from_upload_or_disk(uploaded_computer, "computer.png")
prof_img = load_image_from_upload_or_disk(uploaded_prof, "professor.png")
abijith_img = load_image_from_upload_or_disk(uploaded_abijith, "abijith.jpeg")
dharmayu_img = load_image_from_upload_or_disk(uploaded_dharmayu, "dharmyu.jpeg")
...
with col1:
    ...
    st.markdown("Enter parameters on the left, then press **Generate Topology**.")
    st.markdown("**Topology**: Bus, Star, Ring, Mesh, Tree. Ring variant applies only for Ring.")
    ...
    if generate:
        ...
        # Show summary and step-by-step
        st.subheader("Results & Cost Analysis")
        st.text_area("Summary", value=f"Topology: {topology} {('('+ring_variant+')') if topology=='Ring' else ''}\nNodes: {int(nodes)}\nConnections: {num_edges}\nTotal Cost: ₹{total_cost:.2f}\n\n{step_text}", height=260)
            ...
            for u, v in G.edges():
                ...
                if topology == "Ring" and ring_variant == "Doubly (Unidirectional)":
                    ...
                    if topology == "Ring" and ring_variant == "Singly (Unidirectional)":
                        ...
                        arr = FancyArrow(midpoint[0], midpoint[1], dx*0.0001, dy*0.0001, width=0.008, head_width=0.05, head_length=0.05, color='black', length_includes_head=True, zorder=3)
                        ax.add_patch(arr)
        else:
            # draw bus backbone and T connectors
            nodes_list = list(G.nodes())
            ncount = len(nodes_list)
            xs = [pos[n][0] for n in nodes_list]
            left, right = -0.5, ncount - 0.5
        ...
        # nodes: draw computer icons if available, otherwise nodes
        if computer_img is not None:
            # convert PIL to numpy array for matplotlib OffsetImage
            try:
                comp_arr = np.array(computer_img)
                node_count = len(pos)
                zoom = 0.12 if node_count <= 8 else (0.09 if node_count <= 16 else 0.06)
                for node, (x,y) in pos.items():
                    im = OffsetImage(comp_arr, zoom=zoom)
                    ab = AnnotationBbox(im, (x,y), frameon=False, zorder=4)
                    ax.add_artist(ab)
                    ax.text(x, y+0.02, str(node), fontsize=12, fontweight='bold', ha='center', va='center', color='white', zorder=6)
            except Exception:
        ...
        ax.set_title(f"{topology}" + (f" ({ring_variant})" if topology=="Ring" else ""), fontsize=14, fontweight='bold')
        st.pyplot(fig)
        ...
        def create_docx_bytes():
            ...
            doc.add_heading('Cost Analysis Results', level=1)
            doc.add_paragraph(step_text)

            doc.add_heading('Network Topology Diagram', level=1)
            # save current figure to bytes and embed
            img_stream = BytesIO()
            fig.savefig(img_stream, format='PNG', bbox_inches='tight', dpi=150)
            img_stream.seek(0)
            doc.add_picture(img_stream, width=Inches(6))
            last_par = doc.paragraphs[-1]; last_par.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
Convert topologies (Bus / Star / Ring / Mesh / Tree) into visual graphs and compute port + cable costs.

**Guided by:** Dr. Swaminathan Annadurai
    """)
    # Developed by section with images if available
    cols = st.columns([1,1])
    with cols[0]:
        if abijith_img is not None:
            st.image(abijith_img, width=120, caption="Abijith Thennarasu")
        else:
            st.write("Abijith Thennarasu")
    with cols[1]:
        if dharmayu_img is not None:
            st.image(dharmayu_img, width=120, caption="Dharmayu Jadwani")
        else:
            st.write("Dharmayu Jadwani")

    st.markdown("---")
    if prof_img is not None:
        st.image(prof_img, width=160, caption="Dr. Swaminathan Annadurai")
    else:
        st.write("Guided by: Dr. Swaminathan Annadurai")

    st.markdown("---")
    st.markdown("### Learn More")
    st.markdown("This web app uses NetworkX for graph modeling and Matplotlib for drawing. Use the side panel to adjust nodes, topology and cost parameters.")
    st.markdown("Reference video: https://www.youtube.com/watch?v=zbqrNg4C98U")
